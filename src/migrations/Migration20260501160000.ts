...
export class Migration20260501160000 extends Migration {
  override up(): void | Promise<void> {
    ...
    this.addSql(
      `create index if not exists "crosspost_routes_source_kind_index" on "crosspost_routes" ("source_kind");`,
    );
    this.addSql(
      `create index if not exists "crosspost_routes_source_key_index" on "crosspost_routes" ("source_key");`,
    );
    this.addSql(
      `create index if not exists "crosspost_routes_enabled_source_kind_source_key_index" on "crosspost_routes" ("enabled", "source_kind", "source_key");`,
    );
...
exception when others then null;
    ...
    this.addSql(
      `create table if not exists "crosspost_deliveries" ("id" uuid not null default uuidv7(), "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "route_id" uuid not null, "target_id" text not null, "source_key" text not null, "source_message_id" text not null, "target_message_id" text not null, "deleted_at" timestamptz null default null, primary key ("id"));`,
    );
    this.addSql(
      `create index if not exists "crosspost_deliveries_route_id_index" on "crosspost_deliveries" ("route_id");`,
    );
    this.addSql(
      `create index if not exists "crosspost_deliveries_source_key_index" on "crosspost_deliveries" ("source_key");`,
    );
    this.addSql(`do $$ begin
