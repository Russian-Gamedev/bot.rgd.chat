...
<details>
<summary>2026-07-08 - Member color and event ordering</summary>

- В `BarMember` добавлено поле `color`.
- Во все server-to-client события добавлено поле `seq` для детерминированного порядка обработки.
- В `connected.guilds[]` добавлено поле `voices` со снимком участников публичных голосовых каналов.
...
- `ts` (number) - серверное время события в миллисекундах. Это метаданные времени, а не гарантия порядка.
- `seq` (number) - серверный порядковый номер события. Клиентам, которым нужен детерминированный ввод симуляции, нужно упорядочивать server-to-client события по `seq`, а не по `ts`.

`seq` монотонно растёт внутри одного процесса сервера и может сброситься после рестарта. `seq` не является глобальным между несколькими инстансами.

## События от сервера к клиенту

### Начальная синхронизация

При успешном подключении сервер сначала отправляет `connected`, затем последние broadcast-события (по умолчанию 50, но количество может меняться), затем live-события. Это позволяет клиенту синхронизировать состояние без потери данных.

Replay-события сохраняют исходные `seq` и `ts`, поэтому их `seq` может быть меньше, чем `seq` события `connected`, которое открыло текущее подключение. Клиент должен использовать `connected` как snapshot, а replay/live события применять по `seq`.
...
- `guilds[]` - массив Discord серверов
  ...
  - `members[]` - массив активных участников сервера
    ...
    - `color` (string) - цвет участника в формате hex без `#`
    - `is_bot` (boolean) - `true`, если участник является ботом
  - `voices` (object) - снимок участников публичных голосовых каналов
    - ключ объекта - `channel_id` голосового канала
    - значение - массив `BarMember` в этом голосовом канале
    - содержит только голосовые каналы, доступные роли `@everyone` для просмотра и подключения
    - публичные голосовые каналы без участников передаются как пустой массив
...
```typescript
{
  ...
  "avatar_url": string,   // URL аватара
  "color": string,        // Hex цвет участника без #
